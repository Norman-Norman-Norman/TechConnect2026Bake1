/**
 * Utility functions for calculating and formatting estimated delivery dates on the frontend
 */

// Supplier lead times in business days (matching backend)
...
/**
 * Calculates delivery information for a product
 * @param supplierId - The supplier ID
 * @returns Object containing formatted delivery info
 */
export function getDeliveryInfo(supplierId: number): {
  deliveryDate: Date;
  formattedDate: string;
  displayText: string;
  isSoon: boolean;
} {
  const deliveryDate = calculateDeliveryDate(supplierId);
